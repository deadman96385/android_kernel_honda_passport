...
#define DRA7XX_SRAM_PUB_PA	(OMAP5_SRAM_PA + DRA7XX_HS_SRAM_RSVD)
...
static void __init omap_detect_sram(void)
{
	...
	if (is_sram_locked()) {
		if (cpu_is_omap34xx()) {
			...
			omap_sram_start = DRA7XX_SRAM_PUB_PA + PAGE_SIZE;
			omap_sram_size = SZ_512K - DRA7XX_HS_SRAM_RSVD - PAGE_SIZE; /* 380KB */
#else
...
#ifdef CONFIG_OMAP_INTERCONNECT_BARRIER
			/* Trim the start to keep unique static mapping for the barrier fix */
			omap_sram_start = DRA7XX_SRAM_PUB_PA + PAGE_SIZE;
			omap_sram_size = SZ_512K - DRA7XX_HS_SRAM_RSVD - PAGE_SIZE; /* 380KB */
#else
