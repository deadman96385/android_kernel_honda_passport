			      ...
			      const char **name)
{
	...
	/*
	 * get codec_name, otherwise
	 * get node via "sound-dai = <&phandle port>"
	 * it will be used as xxx_of_node on soc_bind_dai_link()
	 */

	*p_node = NULL;
	ret = of_property_read_string(np, "codec-name", codec_name);
	printk("%s: codec-name: np = %p ret = %d\n", __func__, np, ret);
	if (ret < 0) {
	...
	/* get dai->name */
	ret = of_property_read_string(np, "codec-dai-name", name);
	if (ret < 0) {
