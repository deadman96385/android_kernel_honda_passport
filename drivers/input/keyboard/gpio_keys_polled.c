...
#if defined(CONFIG_MACH_DENSOJ6REF)
		/* XXX: Because drivers/gpio/gpio-densoj6-cpld.c do not follow
		 * linux gpio driver implementation,
		 * that is, <cpld_gpio Y GPIO_LOW_ACTIVE> is not working,
		 * custom implementation is used.
		 */
		u32 lwc_prop[2];

		if (!of_find_property(pp, "lwc-gpios", NULL)) {
			pdata->nbuttons--;
			dev_warn(dev, "Found button without gpios\n");
			continue;
		}

		if (!of_property_read_u32_array(pp, "lwc-gpios",
						&lwc_prop[0], 2)) {
			gpio = lwc_prop[0];
			flags = lwc_prop[1];
