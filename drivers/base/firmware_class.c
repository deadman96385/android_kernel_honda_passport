 ...
 */
/*
 *  This file modified by Honda R&D Americas, Inc. between May 20, 2016
 *  and May 25, 2016.
 *
				      ...
				      const char *buf, size_t count)
{
	...
	case 0:
		if (test_bit(FW_STATUS_LOADING, &fw_buf->status)) {
			 ...
			 * is completed.
			 * */
			err = fw_map_pages_buf(fw_buf);
			if (err) {
				clear_bit(FW_STATUS_DONE, &fw_buf->status);
				fw_load_abort(fw_priv);
				dev_err(dev, "%s: error(%d) occurred!!!\n",
					__func__, err);
				break;
			}
