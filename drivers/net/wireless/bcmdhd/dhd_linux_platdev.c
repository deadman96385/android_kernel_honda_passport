 ...
 *  March 6, 2015 and August 18, 2015
 *
...
extern void wifi_card_detect(void);  /* Murata */
...
int wifi_platform_bus_enumerate(wifi_adapter_info_t *adapter, bool device_present)
{
	...
	wifi_card_detect();	/* Murata -- workaround for card detect */
	return err;
...
static int wifi_plat_dev_drv_probe(struct platform_device *pdev)
{
	...
	DHD_INFO(("%s !!!!!!!! setting up WLAN IRQ !!!!!!!!!!!!!!!!!!!\n", __FUNCTION__));
...
/*	wifi_regulator = regulator_get(&pdev->dev, "wlreg_on");*/
	wifi_regulator = regulator_get(&pdev->dev, "vmmcwl_fixed");   /* use existing TI Wilink code in dra7-evm.dts */
	...
	OOB_PARAM_IF(!dhd_oob_disable) {
		...
		adapter->intr_flags = IORESOURCE_IRQ | IORESOURCE_IRQ_LOWLEVEL |
...
static int dhd_wifi_platform_load_sdio(void)
{
	...
	if (err) {
		DHD_ERROR(("%s: sdio_register_driver timeout or error %d\n", __FUNCTION__, err));
		dhd_bus_unregister();
		goto fail;
	}
