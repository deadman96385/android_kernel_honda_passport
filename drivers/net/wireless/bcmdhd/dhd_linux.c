...
dhd_set_mcast_list_handler(void *handle, void *event_info, u8 event)
{
	...
	if (ap_net_dev != NULL)  {
		...
		if (ifp == NULL || !dhd->pub.up) {
			DHD_ERROR(("%s:%d interface info not available/down \n",
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
...
#if defined(USE_DYNAMIC_F2_BLKSIZE) && defined(CUSTOMER_AUTO_PLAT1)
		dhdsdio_func_blocksize(dhd, 2, DYNAMIC_F2_BLKSIZE_FOR_NONLEGACY);
#endif /* USE_DYNAMIC_F2_BLKSIZE && CUSTOMER_AUTO_PLAT1 */
	...
	if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0)) < 0)
		DHD_ERROR(("%s autocountry failed %d\n", __FUNCTION__, ret));
#endif /* DISABLE_AUTOCOUNTRY */
	...
	printk("%s set credall=0x%X\n", __FUNCTION__, credall);
	bcm_mkiovar("bus:credall", (char *)&credall, 4, iovbuf, sizeof(iovbuf));
	dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0);
#endif

	/* enable rxcb to give more priority to sdio bus. */
	printk("%s set rxcb=0x%X\n", __FUNCTION__, rxcb);
...
dhd_module_init(void)
{
	...
	if (err) {
		DHD_ERROR(("%s module load failed with error %d\n", __FUNCTION__, err));
	}
