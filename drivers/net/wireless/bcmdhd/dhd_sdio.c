 ...
 *
 * $Id: dhd_sdio.c 649723 2016-07-19 07:45:20Z $
 */
...
#if defined(CUSTOMER_AUTO_PLAT1) && defined(USE_DYNAMIC_F2_BLKSIZE)
	...
	int *pad_pkt_len, void **new_pkt)
{
	...
	*new_pkt = NULL;
	...
	if (alloc_new_pkt) {
		...
		bus->dhd->tx_realloc++;
#ifdef CUSTOMER_AUTO_PLAT1
		if (newpkt_size > (2 * SDIO_MAX_BLOCK_SIZE)) {
			tmp_pkt = PKTGET(osh, newpkt_size, TRUE);
			bus->flag = 1;
		}
		else {
			PKTSETDATA_WAR(osh, bus->war_pad_pkt, bus->war_data);
			PKTSETTAIL_WAR(osh, bus->war_pad_pkt, bus->war_tail);
			PKTSETLEN(osh, bus->war_pad_pkt, newpkt_size);
			tmp_pkt = bus->war_pad_pkt;
		}
#else
		tmp_pkt = PKTGET(osh, newpkt_size, TRUE);
#endif /* CUSTOMER_AUTO_PLAT1 */
...
#ifdef CUSTOMER_AUTO_PLAT1
	if (PKTHEAD_WAR(osh, pkt) == bus->war_head) {
		/* Do not free pre-allocated pad packet */
		free_pkt = 0;
	}
#endif /* CUSTOMER_AUTO_PLAT1 */
...
#ifdef CUSTOMER_AUTO_PLAT1
	if (bus->war_pad_pkt)
		PKTFREE(osh, bus->war_pad_pkt, FALSE);
	bus->war_pad_pkt = PKTGET(osh, 2 * SDIO_MAX_BLOCK_SIZE, FALSE);
	bus->war_tail = PKTTAIL_WAR(osh, bus->war_pad_pkt);
	bus->war_end = PKTEND_WAR(osh, bus->war_pad_pkt);
	bus->war_head = PKTHEAD_WAR(osh, bus->war_pad_pkt);
	bus->war_data = PKTDATA_WAR(osh, bus->war_pad_pkt);
	bus->war_truesize = PKTTRUESIZE_WAR(osh, bus->war_pad_pkt);
	bus->war_len = PKTLEN_WAR(osh, bus->war_pad_pkt);
	bus->war_data_len = PKTDATALEN_WAR(osh, bus->war_pad_pkt);
#endif /* CUSTOMER_AUTO_PLAT1 */
