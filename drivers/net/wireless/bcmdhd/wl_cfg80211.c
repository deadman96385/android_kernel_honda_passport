 ...
 *
 * $Id: wl_cfg80211.c 652594 2016-08-02 13:01:17Z $
 */
/* */
#include <typedefs.h>
#include <linuxver.h>
...
#define NL80211_WAPI_VERSION_1	4
#endif
...
#if (MAX_CONNECTIONS > 0)
	/* Set max number of allowed Station connections */
	err = wldev_iovar_getint(dev, "maxassoc", &maxassoc);
	if (err) {
		WL_ERR(("Failed to get max allowed STA connections.\n"));
		goto exit;
	}

	if (MAX_CONNECTIONS > maxassoc) {
		WL_ERR(("Max allowed STA connections %d is out of range,"
				"it should be less than %d.\n",
				MAX_CONNECTIONS, maxassoc));
		goto exit;
	} else if (MAX_CONNECTIONS == maxassoc) {
		WL_INFO(("Max allowed STA connections is already set to %d.\n",
			maxassoc));
		goto exit;
	}

	err = wldev_iovar_setint(dev, "maxassoc", (s32) MAX_CONNECTIONS);
	if (err) {
		WL_ERR(("Failed to set %d max allowed STA connections, error %d\n",
				MAX_CONNECTIONS, err));
	}
#endif /* MAX_CONNECTIONS > 0 */
