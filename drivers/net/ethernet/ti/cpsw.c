 ...
 */
/*
 *  This file is modified by Honda R&D Americas, Inc. between November 21, 2016
...
#include "../../../misc/denso_hw_ctrl/src/nor_api.h"
#endif
...
#ifdef CONFIG_MACH_DENSOJ6REF
static int get_nor_mac(u8 *nor_mac_addr, struct platform_device *pdev){
	int iter = 0;
	char buffer[private_nor.bluetooth_address.size], buf[2]; /* only 2 characters needed */
	memset(buffer, '\0', sizeof(buffer));
	NOR_PrivateRead(private_nor.bluetooth_address.offset, buffer, private_nor.bluetooth_address.size);
	dev_info(&pdev->dev, " buf - %s; addr - 0x%02x; size - %i; \n", buffer, private_nor.bluetooth_address.offset, private_nor.bluetooth_address.size);
	while (iter < ETH_ALEN){
		snprintf(buf, 3, "%s", buffer + (iter*2)); /* Copy chars 2 by 2 from the NOR strings */
		nor_mac_addr[iter] = simple_strtol(buf, NULL, 16);
		iter++;
	}
	return 0;
}
...
#ifdef CONFIG_MACH_DENSOJ6REF
		get_nor_mac(priv->mac_addr, pdev);
#endif
		/* even if the check bellow is redundant, leave for code simplicity */
		if (! is_valid_ether_addr(priv->mac_addr)){
			eth_random_addr(priv->mac_addr);
			dev_info(&pdev->dev, "Random MACID = %pM\n", priv->mac_addr);
		}
		dev_info(&pdev->dev, "Used MACID = %pM\n", priv->mac_addr);
