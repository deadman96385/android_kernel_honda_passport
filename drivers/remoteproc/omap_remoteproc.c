...
static void omap_rproc_kick(struct rproc *rproc, int vqid)
{
	...
	/* No need to do any PM on late-attach */
...
static int omap_rproc_start(struct rproc *rproc)
{
	 ...
	 * manually for balancing it for auto-suspend only for kernel
	 * booted remote processors. Makes no sense to enable PM for a core
	 * that kernel is not responsible of.
	 */
	if (!rproc->late_attach) {
...
static int omap_rproc_stop(struct rproc *rproc)
{
	...
	/*
	 * deny access to remote-processors that were not started by it.
	 * there should be no need for kernel to do PM on a core that is
	 * powered up by u-boot, if PM is required for this core than it
	 * would be done inside the core itself.
	 * In case of late-attached rprocs we want omap_rproc_start to be
	 * called only once at boot and than never again that's why we
	 * return -EACCES so that the omap_rproc_stop will never succeed.
	 * By doing so omap_rproc_start will not be called again.
	 */
	if (rproc->late_attach) {
		return -EACCES;
	}
