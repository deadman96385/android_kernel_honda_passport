...
{
	printk(KERN_ERR "[early camera] init - early camera module not functional in this release %s %d\n",__FUNCTION__,__LINE__);
...
{
	printk(KERN_INFO "[early camera] cleanup \n");
}
...
#include <../drivers/media/platform/ti-vpe/vip.h>
#endif
...
int capture_image(struct file *fp, int init)
{
	...
	if (init) {
		...
		fmt.fmt.pix.width = EARLYCAM_VIP_WIDTH;
		fmt.fmt.pix.height = EARLYCAM_VIP_HEIGHT;
...
int main_fn(void *arg)
{
	...
	while (activated == 1) {
		...
		if (comp.ovls[0].ba != 0) {
			...
			if (ret)
				pr_err("display_queue failed with error %d", ret);
		}
		msleep(8);
	}
...
#else
	int size = 4096000;
	void *mem = kmalloc(size, GFP_USER);
	memset(mem, 0, size);
	comp.ovls[0].ba = (u32) mem;
