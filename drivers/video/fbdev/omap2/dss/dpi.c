...
static int dpi_display_enable(struct omap_dss_device *dssdev)
{
	...
	/*add clock cleaner to fix Alpine disply flicker issue*/
#if defined(CONFIG_OMAP2_CDCE913_CLOCK)
	r = start_cdce913();
	if (r < 0) {
		DSSERR("CANNOT ENABLE CDCE913!!!\n");
		/*
		 * Warn, but do not return an error if the CDCE913 clock
		 * cleaner chip is not present.  It will not cause the rest
		 * of the display initialization problems.
		 */
		r = 0;
	} else
		DSSINFO("++++++++ENABLE CDCE913++++++++\n");
#endif
	return r;
...
static int dpi_init_pll(struct dpi_data *dpi)
{
	struct pll_data *pll;
	int r;
