...
/*PLL_CONFIGURATION1 M4 REGM REGN shift bits*/
#define PLL_CONFIGURATION1_M4_CLOCK_DIV_SHIFT	(21)
#define PLL_CONFIGURATION1_REGM_SHIFT		(9)
#define PLL_CONFIGURATION1_REGN_SHIFT		(1)

#define PLL_CONFIGURATION1_M4_CLOCK_DIV(x)	\
	((x) << PLL_CONFIGURATION1_M4_CLOCK_DIV_SHIFT)
#define PLL_CONFIGURATION1_REGM(x)		\
	((x) << PLL_CONFIGURATION1_REGM_SHIFT)
#define PLL_CONFIGURATION1_REGN(x)		\
	((x) << PLL_CONFIGURATION1_REGN_SHIFT)
		...
		unsigned long clkout_max, pll_calc_func func, void *data)
{
...
#if defined(CONFIG_PANEL_HRALVDS_THRA) && !defined(SPECTRUM_DISPLAY)
	/*temperary hard code HRALVDS THRA display regn and regm to reduce LVDS jitter as TI suggested*/
	for (regn = 119; regn <= regn_stop; ++regn) {
#else
	for (regn = regn_start; regn <= regn_stop; ++regn) {
		...
		regm_stop = min3(clkout_max / fint / 2,
				...
				feats->regm_max);
#if defined(CONFIG_PANEL_HRALVDS_THRA) && !defined(SPECTRUM_DISPLAY)
		for (regm = 1782; regm <= regm_stop; ++regm) {
#else
		for (regm = regm_start; regm <= regm_stop; ++regm) {
#endif
			clkout = 2 * regm * fint;
...
#if defined(CONFIG_PANEL_HRALVDS_TJBA)
	/*use TI latest pll value M4 = 7 M = 1648 N = 99 */
	l = PLL_CONFIGURATION1_M4_CLOCK_DIV(7) |
		PLL_CONFIGURATION1_REGM(1648) |
		PLL_CONFIGURATION1_REGN(99);
#elif defined(CONFIG_PANEL_HRALVDS_THRA) && !defined(SPECTRUM_DISPLAY)
 	/*use TI latest pll value M4 = 3 M = 1485 N = 99 */
	l = PLL_CONFIGURATION1_M4_CLOCK_DIV(3) |
		PLL_CONFIGURATION1_REGM(1485) |
		PLL_CONFIGURATION1_REGN(99);
#endif
