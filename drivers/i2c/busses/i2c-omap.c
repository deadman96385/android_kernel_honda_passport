...
#define OMAP_I2C_IE_ROVR	(1 << 11)	/* RX overrun int enable */
...
struct omap_i2c_dev {
	...
	void			(*set_mpu_wkup_lat)(struct device *dev,
						    long latency);
	int			(*transmit_data)(struct omap_i2c_dev *dev,
							u8 num_bytes,
							bool is_xdr);
			     ...
			     struct i2c_msg *msg, int stop)
{
	...
	/* preload transmitter on write transaction */
	if (! dev->receiver) {
		u8 len;
		len = dev->buf_len < dev->fifo_size ?
			dev->buf_len : dev->fifo_size;
		dev->transmit_data(dev, len, false);
	}
...
/*
 * There are four "transmit_data" functions to simplify the loop that the
 *	original transmit_data function had.  This may be naive, but to check
 *	two conditions that were almost never true didn't make sense, each time
 *	through the loop.
 * So there are two 16 bit write functions (one for I462 and one without) and
 *	there are two 8 bit write functions.  8/16 bit refers to the size of the
 *	data register.
 * We've copied an inherent bug, though.  Just because the register is 16 bits
 *	doesn't necessarily mean the size of the transaction is an even number
 *	of bytes.
 */
static int omap_i2c_transmit_data_i462_16(struct omap_i2c_dev *dev,
	u8 num_bytes, bool is_xdr)
{
	u16 w;
	int ret;
...
static int omap_i2c_transmit_data_i462_8(struct omap_i2c_dev *dev,
	u8 num_bytes, bool is_xdr)
{
	u16 w;
	int ret;
...
static int omap_i2c_transmit_data_16(struct omap_i2c_dev *dev,
	u8 num_bytes, bool is_xdr)
{
	u16		w;
...
static int omap_i2c_transmit_data_8(struct omap_i2c_dev *dev,
	u8 num_bytes, bool is_xdr)
{
	u16		w;
...
omap_i2c_isr_thread(int this_irq, void *dev_id)
{
	...
	do {
		...
		if (stat & OMAP_I2C_STAT_XDR) {
			...
			ret = dev->transmit_data(dev, num_bytes, true);
			if (ret < 0)
				break;
		...
		if (stat & OMAP_I2C_STAT_XRDY) {
			...
			ret = dev->transmit_data(dev, num_bytes, false);
			if (ret < 0)
				break;
...
omap_i2c_probe(struct platform_device *pdev)
{
	...
	/* set the transmit_data function */
	if (dev->flags & I2C_OMAP_ERRATA_I462)
		if (dev->flags & OMAP_I2C_FLAG_16BIT_DATA_REG)
			dev->transmit_data = omap_i2c_transmit_data_i462_16;
		else
			dev->transmit_data = omap_i2c_transmit_data_i462_8;
	else if (dev->flags & OMAP_I2C_FLAG_16BIT_DATA_REG)
		dev->transmit_data = omap_i2c_transmit_data_16;
	else
		dev->transmit_data = omap_i2c_transmit_data_8;
