...
static int serial8250_default_handle_irq(struct uart_port *port)
{
	...
	switch(port->type) {
	case PORT_TI_TL16C75X:
		/** TL16C75x and their brethren
		 *  require LCR[7] == 0 ( DLAB unset )
		 *  to access IIR. Otherwise will access
		 *  either EFR or AFR. This may be true
		 *  for all TI / OMAP devices...
		 */
		          ...
		          struct ktermios *old)
{
	...
	if (up->capabilities & UART_CAP_EFR) {
		...
		/* can't set the TX/RX triggers without ECB */
		if (up->fcr & UART_FCR_TRIGGER_MASK)
			efr |= UART_EFR_ECB;
