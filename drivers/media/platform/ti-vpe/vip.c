			...
			struct v4l2_frmsizeenum *f)
{
	...
	return v4l2_subdev_call(port->subdev, video, enum_framesizes, f);
}
			...
			struct v4l2_frmivalenum *f)
{
	...
	for (fsize.index = 0; ; fsize.index++) {
		ret = v4l2_subdev_call(port->subdev, video,
			     ...
			     struct v4l2_format *f)
{
	...
	ret = v4l2_subdev_call(port->subdev, video, try_mbus_fmt, mf);
	if (ret) {
	...
	vip_dbg(3, dev, "s_fmt subdev try_fmt mbus_code: %04X size: %dx%d\n",
		mf->code,
		mf->width, mf->height);
	ret = v4l2_subdev_call(port->subdev, video, s_mbus_fmt, mf);
	if (ret) {
...
static int vip_init_port(struct vip_port *port)
{
	...
	ret = v4l2_subdev_call(port->subdev, video, g_mbus_fmt, mbus_fmt);
	if (ret)
...
static int vip_setup_parser(struct vip_port *port)
{
	...
	if (endpoint->bus_type == V4L2_MBUS_BT656) {
        ...
        if (VIP_PORTA == port->port_id )  /* DENSO: HDMI port */
	{
		config0 |= VIP_USE_ACTVID_HSYNC_ONLY;
		config0 |= VIP_DISCRETE_BASIC_MODE;
	}else if (VIP_PORTB == port->port_id ) /* DENSO: NTSC camera port */
	{
		config0 &= ~VIP_USE_ACTVID_HSYNC_ONLY;
		config0 |= VIP_FID_DETECT_MODE;
		config0 |= VIP_VSYNC_POLARITY;
		config0 |= 1 << VIP_FID_SKEW_PRECOUNT_SHFT;
		config0 |= 1 << VIP_FID_SKEW_POSTCOUNT_SHFT;
        }
...
static int alloc_port(struct vip_dev *dev, int id)
{
	...
	return get_subdev_active_format(port, port->subdev);
#endif
}
			...
			struct v4l2_async_subdev *asd)
{
	...
	vip_info(dev, "Using subdev %s for capture\n", subdev->name);
...
of_node_cleanup:
	...
	vip_dbg(1, dev, "register async notifier for %d subdevs", i);
...
static int vip_of_probe(struct platform_device *pdev)
{
	...
	while (1) {
		port_node = of_get_next_port(parent, port_node);
		if (!port_node)
			break;
