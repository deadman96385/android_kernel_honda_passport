...
/*********************************************************
 *
 *  This file modified by Honda R&D Americas, Inc. on December 6, 2016
 *
...
#define VPE_MAX_DMA_POOL_BUF_PREALLOC (8)

static int dma_pool_buf_size;
static int dma_pool_prealloc;
...
static void *vpe_dma_alloc(struct vpe_ctx *ctx, size_t buf_size,
	dma_addr_t *dma_handle)
{
	if (dma_pool_buf_size == buf_size && ctx->dev->dma_pool)
		return dma_pool_alloc(ctx->dev->dma_pool, GFP_KERNEL,
					dma_handle);
	else
		return dma_alloc_coherent(ctx->dev->v4l2_dev.dev, buf_size,
						dma_handle, GFP_KERNEL);
}

static void vpe_dma_free(struct vpe_ctx *ctx, size_t buf_size, void *vaddr,
	dma_addr_t dma_handle)
{
	if (ctx->mv_buf_size == dma_pool_buf_size && ctx->dev->dma_pool)
		dma_pool_free(ctx->dev->dma_pool, vaddr, dma_handle);
	else
		dma_free_coherent(ctx->dev->v4l2_dev.dev, buf_size,
					vaddr, dma_handle);
}
...
static int realloc_mv_buffers(struct vpe_ctx *ctx, size_t size)
{
	...
	if (ctx->mv_buf[0])
		vpe_dma_free(ctx, ctx->mv_buf_size, ctx->mv_buf[0],
				ctx->mv_buf_dma[0]);

	if (ctx->mv_buf[1])
		vpe_dma_free(ctx, ctx->mv_buf_size, ctx->mv_buf[1],
				ctx->mv_buf_dma[1]);

	if (size == 0)
		return 0;

	ctx->mv_buf[0] = vpe_dma_alloc(ctx, size, &ctx->mv_buf_dma[0]);
	...
	ctx->mv_buf[1] = vpe_dma_alloc(ctx, size, &ctx->mv_buf_dma[1]);
...
static void vpe_dma_pool_init(struct platform_device *pdev)
{
	struct vpe_dev *dev = platform_get_drvdata(pdev);
	int i, n;

	if (!dma_pool_buf_size)
		return;

	dev->dma_pool = dma_pool_create(VPE_MODULE_NAME "_dma_pool",
		dev->v4l2_dev.dev, dma_pool_buf_size, PAGE_SIZE, 0);
	if (!dev->dma_pool) {
		dev_err(dev->v4l2_dev.dev, "Failed to create DMA pool "
			"of size %u\n", dma_pool_buf_size);
		return;
	}

	n = dma_pool_prealloc > VPE_MAX_DMA_POOL_BUF_PREALLOC ?
		VPE_MAX_DMA_POOL_BUF_PREALLOC : dma_pool_prealloc;

	if (n) {
		dma_addr_t dma_addr[n];
		void *dma_buf[n];

		dev_info(dev->v4l2_dev.dev, "Preallocating %u DMA "
			"buffers of size %u\n", n, dma_pool_buf_size);

		/* Preallocate DMA buffers into the pool */
		for (i = 0; i < n; i++) {
			dma_buf[i] = dma_pool_alloc(dev->dma_pool, GFP_KERNEL,
						&dma_addr[i]);
			if (!dma_buf[i]) {
				dev_err(dev->v4l2_dev.dev, "Failed to "
					"preallocate DMA buffer %u/%u\n",
					i + 1, n);
				break;
			}
		}
		...
		/*
		 * Release the DMA buffers, but pages allocated for the
		 * buffers remain in the pool.
		 */
		for (i--; i >= 0; i--)
			dma_pool_free(dev->dma_pool, dma_buf[i], dma_addr[i]);
	}
}
...
module_param(dma_pool_buf_size, int, 0444);
MODULE_PARM_DESC(dma_pool_buf_size,
	"Use a DMA pool to allocate buffers of this size.");
module_param(dma_pool_prealloc, int, 0444);
MODULE_PARM_DESC(dma_pool_prealloc,
	"Number of DMA buffers to pre-allocate from DMA pool.");
