...
static bool adv7604_has_afe(struct adv76xx_state *state)
{
...
static const struct v4l2_dv_timings adv7611_timings[] = {
...
static s32 adv_smbus_read_byte_data_check_318(struct i2c_client *client,
		u8 command, bool check)
{
	union i2c_smbus_data data;
...
static s32 adv_smbus_read_byte_data_318(struct adv76xx_state *state,
				    enum adv76xx_page page, u8 command)
{
	return adv_smbus_read_byte_data_check_318(state->i2c_clients[page],
					      command, true);
}

static s32 adv_smbus_write_byte_data_new(struct adv76xx_state *state,
				     enum adv76xx_page page, u8 command,
				     u8 value)
{
...
static s32 adv_smbus_read_byte_data_new(struct adv76xx_state *state,
				    enum adv76xx_page page, u8 command)
{
...
{
    return &container_of(ctrl->handler, struct adv76xx_state, hdl)->sd;
}
...
/* ----------------------------------------------------------------------- */
static int regmap_read_check(struct adv76xx_state *state,
 ...
 */
static int regmap_write_block(struct adv76xx_state *state,
					  enum adv76xx_page page, u8 command,
					  size_t length, const u8 *values)
{
	struct regmap *regmap = state->regmap[page];

	if (length > I2C_SMBUS_BLOCK_MAX)
		length = I2C_SMBUS_BLOCK_MAX;

	return regmap_raw_write(regmap, command, values, length);
}
...
static inline int io_read(struct v4l2_subdev *sd, u8 reg)
{
	...
	return regmap_read_check(state, ADV76XX_PAGE_IO, reg);
}
...
static inline int cec_read(struct v4l2_subdev *sd, u8 reg)
{
	...
	return regmap_read_check(state, ADV76XX_PAGE_CEC, reg);
}
...
static inline int infoframe_read(struct v4l2_subdev *sd, u8 reg)
{
	...
	return regmap_read_check(state, ADV76XX_PAGE_INFOFRAME, reg);
}
...
static inline int afe_read(struct v4l2_subdev *sd, u8 reg)
{
	...
	return regmap_read_check(state, ADV76XX_PAGE_AFE, reg);
}
...
static inline int rep_read(struct v4l2_subdev *sd, u8 reg)
{
	...
	return regmap_read_check(state, ADV76XX_PAGE_REP, reg);
}
...
static inline int edid_read(struct v4l2_subdev *sd, u8 reg)
{
	...
	return regmap_read_check(state, ADV76XX_PAGE_EDID, reg);
}
					...
					unsigned len, const u8 *val)
{
	...
	for (i = 0; !err && i < len; i += I2C_SMBUS_BLOCK_MAX)
		err = regmap_write_block(state, ADV76XX_PAGE_EDID,
...
static void adv76xx_set_hpd(struct adv76xx_state *state, unsigned int hpd)
{
	...
	v4l2_subdev_notify(&state->sd, HDMI_PLUG, &hpd);
}
...
static inline int hdmi_read(struct v4l2_subdev *sd, u8 reg)
{
	...
	return regmap_read_check(state, ADV76XX_PAGE_HDMI, reg);
}
...
static inline int cp_read(struct v4l2_subdev *sd, u8 reg)
{
	...
	return regmap_read_check(state, ADV76XX_PAGE_CP, reg);
}
...
static int adv76xx_read_reg(struct v4l2_subdev *sd, unsigned int reg)
{
	...
	return regmap_read_check(state, page, reg);
}
#endif

static int adv76xx_write_reg(struct v4l2_subdev *sd, unsigned int reg, u8 val) //TODO
{
	struct adv76xx_state *state = to_state(sd);
	unsigned int page = reg >> 8;
...
static const struct adv7611_format_info adv7604_formats[] =
{
...
static const struct adv7611_format_info adv7611_formats[] =
{
    {V4L2_MBUS_FMT_RGB888_1X24, ADV7604_OP_CH_SEL_RGB, true, false,
            ADV7604_OP_MODE_SEL_SDR_444 | ADV7604_OP_FORMAT_SEL_8BIT },
	{V4L2_MBUS_FMT_YUYV8_2X8, ADV7604_OP_CH_SEL_RGB, false, false,
			ADV7604_OP_MODE_SEL_SDR_422 | ADV7604_OP_FORMAT_SEL_8BIT },
	{V4L2_MBUS_FMT_UYVY8_2X8, ADV7604_OP_CH_SEL_RGB, false, false,
			ADV7604_OP_MODE_SEL_SDR_422 | ADV7604_OP_FORMAT_SEL_8BIT },
    {V4L2_MBUS_FMT_YVYU8_2X8, ADV7604_OP_CH_SEL_RGB, false, true,
            ADV7604_OP_MODE_SEL_SDR_422 | ADV7604_OP_FORMAT_SEL_8BIT },
    {V4L2_MBUS_FMT_YUYV12_2X12, ADV7604_OP_CH_SEL_RGB, false, false,
...
static void adv76xx_set_offset(struct v4l2_subdev *sd, bool auto_offset, u16 offset_a, u16 offset_b, u16 offset_c)
{
	...
	if (regmap_write_block(state, ADV76XX_PAGE_CP, 0x77, 4, offset_buf))
...
static void adv76xx_set_gain(struct v4l2_subdev *sd, bool auto_gain, u16 gain_a, u16 gain_b, u16 gain_c)
{
	...
	/* Registers must be written in this order with no i2c access in between */
	if (regmap_write_block(state, ADV76XX_PAGE_CP, 0x73, 4, gain_buf))
...
static int adv76xx_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_ADV_RX_FREE_RUN_COLOR:
		...
		return 0;
	case V4L2_CID_PRIVATE_HDMI_PORT_ID:
	case V4L2_CID_PRIVATE_HDMI_DEVICE_ID:
		return 0;
	}
	return -EINVAL;
}
		...
		struct v4l2_dv_timings *timings)
{
	...
	for (i = 0; adv7611_timings[i].bt.height; i++) {
		if (vtotal(&adv7611_timings[i].bt) != stdi->lcf + 1)
			continue;
		if (adv7611_timings[i].bt.vsync != stdi->lcvs)
			continue;

		pix_clk = hfreq * htotal(&adv7611_timings[i].bt);

		if ((pix_clk < adv7611_timings[i].bt.pixelclock + 1000000) &&
		    (pix_clk > adv7611_timings[i].bt.pixelclock - 1000000)) {
			*timings = adv7611_timings[i];
			return 0;
		}
	}
		...
		struct v4l2_dv_timings *timings)
{
	...
	for (i = 0; adv7611_timings[i].bt.width; i++) {
		if (v4l2_match_dv_timings(timings, &adv7611_timings[i],
					is_digital_input(sd) ? 250000 : 1000000)) {
			*timings = adv7611_timings[i];
			break;
		}
...
static int adv7611_query_dv_timings(struct v4l2_subdev *sd,
			struct v4l2_dv_timings *timings)
{
	struct adv76xx_state *state = to_state(sd);
	const struct adv76xx_chip_info *info = state->info;
...
found:
	...
	if (debug > 1)
		v4l2_print_dv_timings(sd->name, "adv7611_query_dv_timings: ",
				      timings, true);
...
static int adv7611_s_dv_timings(struct v4l2_subdev *sd,
		struct v4l2_dv_timings *timings)
{
	struct adv76xx_state *state = to_state(sd);
	...
	if (debug > 1)
		v4l2_print_dv_timings(sd->name, "adv7611_s_dv_timings: ",
				      timings, true);
	return 0;
}
...
static int adv7611_enum_mbus_fmt(struct v4l2_subdev *sd, unsigned int index,
...
static int adv7611_enum_framesizes(struct v4l2_subdev *sd,
            struct v4l2_frmsizeenum *fsize)
{
    ...
    /*v4l2_info(client, "adv7611_enum_framesizes \n");*/
...
static int adv7611_g_fmt(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *fmt)
{
    struct adv76xx_state *state = to_state(sd);
    //struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static int adv7611_s_fmt(struct v4l2_subdev *sd,
        struct v4l2_mbus_framefmt *mf)
{
    struct adv76xx_state *state = to_state(sd);

	v4l2_info(&state->sd, "adv7611_s_fmt: Currently only V4L2_PIX_FMT_RGB24 & V4L2_PIX_FMT_YVYU is supported!\n");
...
static const struct v4l2_subdev_video_ops adv76xx_video_ops = {
    .enum_framesizes  = adv7611_enum_framesizes,
    .g_mbus_fmt       = adv7611_g_fmt,
    .s_mbus_fmt       = adv7611_s_fmt,
    .try_mbus_fmt     = adv7611_g_fmt,
    .enum_mbus_fmt    = adv7611_enum_mbus_fmt,
    
	.s_stream         = adv7611_s_stream,            //TODO
	.s_routing        = adv76xx_s_routing,
	.g_input_status   = adv76xx_g_input_status,
	.s_dv_timings     = adv76xx_s_dv_timings,
	.g_dv_timings     = adv76xx_g_dv_timings,
	.query_dv_timings = adv76xx_query_dv_timings,
	.s_dv_timings     = adv7611_s_dv_timings,       //TODO
	.g_dv_timings     = adv7604_g_dv_timings,       //TODO
	.query_dv_timings = adv7611_query_dv_timings,   //TODO
};
...
static const struct v4l2_ctrl_config adv76xx_ctrl_hdmi_port_id = {
	.ops = &adv76xx_ctrl_ops,
	.id = V4L2_CID_PRIVATE_HDMI_PORT_ID,
	.name = "HDMI Port Id",
	.type = V4L2_CTRL_TYPE_INTEGER,
	.min = 0x0,
	.max = 0x1,
...
static const struct v4l2_ctrl_config adv76xx_ctrl_hdmi_device_id = {
	.ops = &adv76xx_ctrl_ops,
	.id = V4L2_CID_PRIVATE_HDMI_DEVICE_ID,
	.name = "HDMI Device Id",
	.type = V4L2_CTRL_TYPE_INTEGER,
	.min = 0x0,
	.max = 0x2051,
...
static const struct adv7604_reg_seq adv7604_recommended_settings_afe[] =
{
    ...
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x0d), 0x04 }, /* HDMI filter optimization */
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x0d), 0x04 }, /* HDMI filter optimization */
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x3d), 0x00 }, /* DDC bus active pull-up control */
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x3e), 0x74 }, /* TMDS PLL optimization */
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x4e), 0x3b }, /* TMDS PLL optimization */
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x57), 0x74 }, /* TMDS PLL optimization */
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x58), 0x63 }, /* TMDS PLL optimization */
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x8d), 0x18 }, /* equaliser */
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x8e), 0x34 }, /* equaliser */
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x93), 0x88 }, /* equaliser */
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x94), 0x2e }, /* equaliser */
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x96), 0x00 }, /* enable automatic EQ changing */

    /* set ADI recommended settings for digitizer */
    /* "ADV7604 Register Settings Recommendations (rev. 2.5, June 2010)" p. 17. */
    { ADV7604_REG(ADV76XX_PAGE_AFE, 0x12), 0x7b }, /* ADC noise shaping filter controls */
    { ADV7604_REG(ADV76XX_PAGE_AFE, 0x0c), 0x1f }, /* CP core gain controls */
    { ADV7604_REG(ADV76XX_PAGE_CP, 0x3e), 0x04 }, /* CP core pre-gain control */
    { ADV7604_REG(ADV76XX_PAGE_CP, 0xc3), 0x39 }, /* CP coast control. Graphics mode */
    { ADV7604_REG(ADV76XX_PAGE_CP, 0x40), 0x5c }, /* CP core pre-gain control. Graphics mode */
...
static const struct adv7604_reg_seq adv7604_recommended_settings_hdmi[] =
{
    ...
    /* "ADV7604 Register Settings Recommendations (rev. 2.5, June 2010)" p. 4. */
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x0d), 0x84 }, /* HDMI filter optimization */
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x3d), 0x10 }, /* DDC bus active pull-up control */
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x3e), 0x39 }, /* TMDS PLL optimization */
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x4e), 0x3b }, /* TMDS PLL optimization */
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x57), 0xb6 }, /* TMDS PLL optimization */
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x58), 0x03 }, /* TMDS PLL optimization */
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x8d), 0x18 }, /* equaliser */
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x8e), 0x34 }, /* equaliser */
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x93), 0x8b }, /* equaliser */
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x94), 0x2d }, /* equaliser */
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x96), 0x01 }, /* enable automatic EQ changing */

    /* reset ADI recommended settings for digitizer */
    /* "ADV7604 Register Settings Recommendations (rev. 2.5, June 2010)" p. 17. */
    { ADV7604_REG(ADV76XX_PAGE_AFE, 0x12), 0xfb }, /* ADC noise shaping filter controls */
    { ADV7604_REG(ADV76XX_PAGE_AFE, 0x0c), 0x0d }, /* CP core gain controls */

    { ADV7604_REG_SEQ_TERM, 0 },
};

static const struct adv7611_reg_seq adv7611_recommended_settings_hdmi[] =
{
    /* ADV7611 Register Settings Recommendations Rev 1.5, May 2014 */
    { ADV7604_REG(ADV76XX_PAGE_CP,   0x6c), 0x00 },
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x9b), 0x03 },
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x6f), 0x08 },
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x85), 0x1f },
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x87), 0x70 },
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x57), 0xda },
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x58), 0x01 },
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x03), 0x98 },
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x4c), 0x44 },
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x8d), 0x04 },
    { ADV7604_REG(ADV76XX_PAGE_HDMI, 0x8e), 0x1e },
...
static struct of_device_id adv76xx_of_id[] __maybe_unused = {
	{ .compatible = "ad,adv7611", .data = &adv76xx_chip_info[ADV7611] },
	{ }
};
...
static void adv7611_adi_recommended_setting(struct v4l2_subdev *sd)
{
			io_write(sd, 0x01, 0x06);	  // Prim_Mode = 110b 60HZ HDMI-GR
			io_write(sd, 0x02, 0xF5);	  // Input color space depends on color space reported by HDMI block
			io_write(sd, 0x03, 0x80);	  // 16 bit SDR 422 mode 0
//			io_write(sd, 0x03, 0x40);	  // 24 bit SDR 444 mode 0
//			io_write(sd, 0x03, 0x00);	  // 8-bit SDR ITU-656 mode
			io_write(sd, 0x04, 0x60);	  // Jackie added ??
			...
			rep_write(sd,   0x40, 0x81); 	//	Disable HDCP 1.1 feature
			...
			hdmi_write(sd,  0x03, 0x98); 		  // DIS_I2C_ZERO_COMPR
			hdmi_write(sd,  0x75, 0x10); 		  // DDC drive strength

}

static void adv76xx_unregister_snd(struct adv76xx_state *state)
{
	if (state != NULL && state->pdev_snd_codec != NULL)
		platform_device_unregister(state->pdev_snd_codec);
}

static int adv76xx_configure_snd(struct adv76xx_state *state)
{
	...
	/* Check i2c-client */
	if (state->i2c_clients[ADV76XX_PAGE_IO] == NULL)
		return -ENODEV;

	client = state->i2c_clients[ADV76XX_PAGE_IO];
	...
	pdev = platform_device_register_resndata(&client->dev,
						 ...
						 PLATFORM_DEVID_NONE,
						 NULL, 0,
						 state, sizeof(struct adv76xx_state));
			 ...
			 const struct i2c_device_id *id)
{
    ...
    printk("adv7611 support: +adv76xx_probe()\n");

    /* Check if the adapter supports the needed features */
    if (!i2c_check_functionality(client->adapter, I2C_FUNC_SMBUS_BYTE_DATA))
        return -EIO;
    v4l_dbg(1, debug, client, "detecting adv7611 client on address 0x%x\n", 
		    client->addr << 1);

    state = devm_kzalloc(&client->dev, sizeof(*state), GFP_KERNEL);  
    if (!state) {
        v4l_err(client, "Could not allocate adv7611_state memory!\n");
        return -ENOMEM;
    }
    ...
    state->format = adv76xx_format_info(state, V4L2_MBUS_FMT_UYVY8_2X8);
    ...
    if (state->info->type == ADV7604) {
        ...
        if (val != 0x2051) {
            ...
            v4l2_info(sd, "adv7611 chip detected on address 0x%x\n", client->addr << 1);
        }
    }

    /* control handlers */
    hdl = &state->hdl;
    v4l2_ctrl_handler_init(hdl, adv76xx_has_afe(state) ? 11 : 10);
    ...
    if(!state->detect_tx_5v_ctrl){
       v4l2_info(sd, "state->detect_tx_5v_ctrl 0x%p\n", state->detect_tx_5v_ctrl);  // JACKIE testing
       return -ENODEV; 
	}
	...
	/* THRACB-154 */
    state->hdmi_port_id_ctrl =
		v4l2_ctrl_new_custom(hdl, &adv76xx_ctrl_hdmi_port_id, NULL);

    state->hdmi_device_id_ctrl =
		v4l2_ctrl_new_custom(hdl, &adv76xx_ctrl_hdmi_device_id, NULL);
    ...
    state->hdmi_port_id_ctrl->is_private = true;
    state->hdmi_device_id_ctrl->is_private = true;
...
#if 1	
		adv7611_adi_recommended_setting(sd);
#endif
	
    /* V4l2 asyn subdev register */
    err = v4l2_async_register_subdev(sd);
    if (!err)
        v4l2_info(&state->sd, "adv7611 driver registered!\n");
    else
        goto err_sound;
