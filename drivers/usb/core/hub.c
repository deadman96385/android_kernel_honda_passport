...
int usb_new_device(struct usb_device *udev)
{
	...
	if (udev->quirks & USB_QUIRK_WAIT_SC_COMPLETE) {
		udev->sc_done = 0;
		init_completion(&udev->sc_complete);
	}
...
#ifdef CONFIG_USB_DISABLE_HUBS_ON_EXT_PORTS
		/* gen1Honda
		 * Using hub device attached to root hub is not allowed
		 * especially for external ports [Bus#3 and Bus#4].
		 * Since userspace app needs to recognize if hub is attached
		 * via uevent messages, we will let hub device be connected,
		 * so that userspace app will notify user not to attach hub.
		 * But, devices under the hub will be discarded with this code.
		 */
		if ((udev->bus->busnum == 3 || udev->bus->busnum == 4) &&
		    udev->level >= 2) {
			dev_err(&udev->dev,
				"can't connect device to this hub port\n");
			status = -ENOTSUPP;     /* Don't retry */
			goto loop_disable;
		}
#endif
