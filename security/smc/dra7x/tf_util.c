	...
	case TF_MESSAGE_TYPE_REGISTER_SHARED_MEMORY:
		...
		if (soc_is_omap54xx() || soc_is_dra7xx()) {
			dprintk(KERN_INFO
				...
				"   shared_mem_descriptors[0] = 0x%016llX\n"
				"   shared_mem_descriptors[1] = 0x%016llX\n"
				"   shared_mem_descriptors[2] = 0x%016llX\n"
				"   shared_mem_descriptors[3] = 0x%016llX\n"
				"   shared_mem_descriptors[4] = 0x%016llX\n"
				"   shared_mem_descriptors[5] = 0x%016llX\n"
				"   shared_mem_descriptors[6] = 0x%016llX\n"
				"   shared_mem_descriptors[7] = 0x%016llX",
				command->header.message_size,
				command->header.message_type,
				...
				(u64)(command->register_shared_memory.
...
#endif
		{
			dprintk(KERN_INFO
				...
				command->register_shared_memory.
					shared_mem_start_offset,
				(u32)(command->register_shared_memory.
					shared_mem_descriptors)[0],
				(u32)(command->register_shared_memory.
					shared_mem_descriptors)[1],
				(u32)(command->register_shared_memory.
		...
		for (i = 0; i < command->header.message_size; i++)
			dprintk(KERN_ERR "command[%02x]=%08x\n", i,
				((uint32_t *)command)[i]);
	...
	case TF_MESSAGE_TYPE_INVOKE_CLIENT_COMMAND:
		...
		for (i = 0; i < 4; i++) {
			uint32_t *param = (uint32_t *) &command->
				open_client_session.params[i];
			dprintk(KERN_INFO
				"   params[%d] = 0x%08X:0x%08X:0x%08X:0x%08X\n",
				i, param[0], param[1], param[2], param[3]);
		}
...
int tf_get_current_process_hash(void *hash)
{
	...
	if (mm && mm->exe_file != NULL) {
			...
			(void) vm_munmap(start, dentry->d_inode->i_size);
			dput(dentry);
		}
vma_out:
