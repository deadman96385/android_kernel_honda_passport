...
void TEEC_encode_parameters_lpae(u16 *param_types,
				 union tf_command_param_lpae *params,
			  ...
			  TEEC_Operation *operation)
{
	if (operation != NULL) {
		...
		/* Flush the assignment to imp._pSession, so that
			RequestCancellation can read that field
			if started==1. */
		barrier();
		operation->started = 1;
	}
					...
					TEEC_Context *context)
{
	...
	if (error != 0) {
		dprintk(KERN_ERR
			"%s(%s): tf_create_device_context failed (error %d)!\n",
			__func__,
			(name == NULL ? "(null)" : name), error);
		goto error;
	}
						...
						TEEC_SharedMemory *sharedMem)
{
	...
	struct tf_command_register_shared_memory_schannel *cmd =
	...
	shmem_addr = (u32)sharedMem->buffer;
						...
						TEEC_SharedMemory *sharedMem)
{
	...
	if (sharedMem->size == 0) {
		/* Allocating 0 bytes must return a non-NULL pointer, but the
			...
			anywhere. So we return a pointer
		...
		if (sharedMem->buffer == NULL) {
			dprintk(KERN_INFO "%s: could not allocate %lu bytes",
				__func__, (unsigned long)sharedMem->size);
					 ...
					 u32 *errorOrigin)
{
	union tf_command command_message = { { 0, } };
#ifdef SUPPORT_LARGE_PHYSICAL_ADDRESS_EXTENSION
	struct tf_command_open_client_session_lpae *cmd_lpae =
		&command_message.open_client_session_lpae;
#endif
	...
	if (soc_is_omap54xx() || soc_is_dra7xx()) {
		cmd_lpae->message_size = TF_COMMAND_SIZE(*cmd_lpae);
		cmd_lpae->message_type = TF_MESSAGE_TYPE_OPEN_CLIENT_SESSION;
		cmd_lpae->operation_id = (u32)&answer_message;
		cmd_lpae->device_context = (u32)context;
		cmd_lpae->cancellation_id = (u32)operation;
		cmd_lpae->timeout = TEEC_encode_timeout(timeLimit);
		memcpy(&cmd_lpae->destination_uuid, destination,
				 sizeof(cmd_lpae->destination_uuid));
		cmd_lpae->login_type = connectionMethod;
		TEEC_encode_parameters_lpae(&cmd_lpae->param_types, cmd_lpae->params,
			operation);
	} else
#endif
	{
					 ...
					 u32 *errorOrigin)
{
	...
	union tf_command command_message = { { 0, } };
#ifdef SUPPORT_LARGE_PHYSICAL_ADDRESS_EXTENSION
	struct tf_command_invoke_client_command_lpae *cmd_lpae =
		&command_message.invoke_client_command_lpae;
#endif
	...
	if (soc_is_omap54xx() || soc_is_dra7xx()) {
		cmd_lpae->message_size = TF_COMMAND_SIZE(*cmd_lpae);
		cmd_lpae->message_type = TF_MESSAGE_TYPE_INVOKE_CLIENT_COMMAND;
		cmd_lpae->operation_id = (u32)&answer_message;
		cmd_lpae->device_context = (u32)context;
		cmd_lpae->client_session = session->imp._client_session;
		cmd_lpae->timeout = TEEC_encode_timeout(timeLimit);
		cmd_lpae->cancellation_id = (u32)operation;
		cmd_lpae->client_command_identifier = commandID;
		TEEC_encode_parameters_lpae(&cmd_lpae->param_types, cmd_lpae->params,
			operation);
	} else
#endif
