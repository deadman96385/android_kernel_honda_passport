	...
	bool wait)
{
	...
	if (ret != TEEC_SUCCESS) {
		printk(KERN_ERR "%s: InvokeCommand OPEN_SESSION returned 0x%08x\n",
			__func__, ret);
		err = -EFAULT;
		goto exit;
	}
	...
	ret = TEEC_InvokeCommand(&teec_session, cmd, &op, NULL);
	if (ret != TEEC_SUCCESS) {
		printk(KERN_ERR "%s: InvokeCommand GENERATERANDOM returned 0x%08x\n",
			__func__, ret);
		err = -EFAULT;
	}
	...
	ret = TEEC_InvokeCommand(&teec_session, cmd, &op, NULL);
	if (ret != TEEC_SUCCESS) {
		printk(KERN_ERR "%s: InvokeCommand CLOSE_SESSION returned 0x%08x\n",
			__func__, ret);
		err = -EFAULT;
	}
