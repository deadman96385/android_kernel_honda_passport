		...
		u32 *descriptor_count)
{
	...
	dprintk(KERN_INFO "tf_map_shmem(%p, 0x%08llx, flags = 0x%08x)\n",
		connection, (u64)buffer, flags);
...
static int tf_map_temp_shmem_lpae(struct tf_connection *connection,
	 struct tf_command_param_temp_memref_lpae *temp_memref,
	 u32 param_type,
	 struct tf_shmem_desc **shmem_desc)
{
	u32 flags;
	...
	dprintk(KERN_INFO "tf_map_temp_shmem(%p, "
		"0x%08llx[size=0x%08x], offset=0x%08x)\n",
		connection,
		temp_memref->descriptor,
		temp_memref->size,
		temp_memref->offset);
	...
	if (temp_memref->descriptor == 0) {
		...
		*shmem_desc = NULL;
		...
		/* Map the temp shmem block */
		u32 descriptor_count;
		/* TODO : add 64 bits descriptor */
		u64 shared_mem_descriptors[TF_MAX_COARSE_PAGES_LPAE];

		if (in_user_space) {
			error = tf_validate_shmem_and_flags(
				(u32)temp_memref->descriptor,
				temp_memref->size,
				flags);
			if (error != 0)
				goto error;
...
int tf_init_shared_memory(struct tf_connection *connection)
{
	 ...
	 */
	for (i = 0; i < 3; i++) {
		u32 max_pages = TF_MAX_COARSE_PAGES;
		struct tf_shmem_desc *shmem_desc =
		...
		if (soc_is_omap54xx() || soc_is_dra7xx())
			max_pages = TF_MAX_COARSE_PAGES_LPAE;
#endif

		for (coarse_page_index = 0;
			  coarse_page_index < max_pages;
			  coarse_page_index++) {
			...
			if (coarse_pg_table == NULL) {
				printk(KERN_ERR
					"%s(%p): desc %d page %d failed\n",
					__func__, connection, i,
					coarse_page_index);
				error = -ENOMEM;
				goto error;
			}
...
static bool tf_check_gid(gid_t requested_gid)
{
	...
	kgid_t requested_kgid = make_kgid(ns, requested_gid);
	if (gid_eq(requested_kgid, current_egid())) {
		return true;
	} else {
		u32	 size;
		u32	 i;
		/* Look in the supplementary GIDs */
		get_group_info(get_current_groups());
		size = get_current_groups()->ngroups;
		for (i = 0; i < size; i++)
			if (gid_eq(requested_kgid, GROUP_AT(get_current_groups() , i)))
				return true;
	}
	return false;
}
	...
	union tf_answer *answer)
{
	...
	if (soc_is_omap54xx() || soc_is_dra7xx()) {
		command->open_client_session.message_size =
			(sizeof(struct tf_command_open_client_session_lpae) - 20
			- sizeof(struct tf_command_header))/4;
		login_type = &command->open_client_session_lpae.login_type;
		login_data = &command->open_client_session_lpae.login_data[0];
	} else
#endif
	{
		command->open_client_session.message_size =
			(sizeof(struct tf_command_open_client_session) - 20
				- sizeof(struct tf_command_header))/4;
		login_type = &command->open_client_session.login_type;
		login_data = &command->open_client_session.login_data[0];
	}
	...
	case TF_LOGIN_USER:
		...
		*(u32 *) login_data = (u32) __kuid_val(current_euid());

#ifndef CONFIG_ANDROID
		*login_type = (u32) TF_LOGIN_USER_LINUX_EUID;
#else
		*login_type = (u32) TF_LOGIN_USER_ANDROID_EUID;
#endif
	...
	case TF_LOGIN_GROUP: {
		/* Check requested GID */
		gid_t  requested_gid = *(u32 *) login_data;
...
#ifndef CONFIG_ANDROID
		*login_type = TF_LOGIN_GROUP_LINUX_GID;
#else
		*login_type = TF_LOGIN_GROUP_ANDROID_GID;
#endif
	...
	case TF_LOGIN_APPLICATION: {
		...
		memcpy(login_data, pSHA1Hash, 16);
		*login_type = TF_LOGIN_APPLICATION_LINUX_PATH_SHA1_HASH;
		/* 16 bytes */
	...
	case TF_LOGIN_APPLICATION:
		 ...
		 * data. Update message size.
		 */
		*(u32 *) login_data = (u32) __kuid_val(current_uid());

		*login_type = (u32) TF_LOGIN_APPLICATION_ANDROID_UID;
	...
	case TF_LOGIN_APPLICATION_USER:
		 ...
		 */
		*((u32 *)login_data) = (u32) __kuid_val(current_uid());
		*((u32 *)(login_data+4)) = (u32) __kuid_val(current_euid());
	...
	case TF_LOGIN_APPLICATION_GROUP: {
		...
		if (!tf_check_gid(requested_gid)) {
			...
			"does not match real eGID (0x%x)"
			"or any of the supplementary GIDs\n",
			connection, requested_gid, (u32) __kgid_val(current_egid()));
			error = -EACCES;
			goto error;
		}

		*((u32 *)login_data) = (u32) __kuid_val(current_uid());
		*((u32 *)(login_data+4)) = requested_gid;
	...
	case TF_LOGIN_PRIVILEGED:
		...
		} else if ((!uid_eq(current_euid(), KUIDT_INIT(TF_PRIVILEGED_UID_GID))) &&
				(!gid_eq(current_egid(), KGIDT_INIT(TF_PRIVILEGED_UID_GID))) &&
				(!uid_eq(current_euid(), GLOBAL_ROOT_UID)) &&
				(!gid_eq(current_egid(), GLOBAL_ROOT_GID))) {
			dprintk(KERN_ERR "tf_open_client_session: "
		...
		*login_type = TF_LOGIN_PRIVILEGED;
		break;
...
#ifdef SUPPORT_LARGE_PHYSICAL_ADDRESS_EXTENSION
			if (soc_is_omap54xx() || soc_is_dra7xx()) {
				error = tf_map_temp_shmem_lpae(connection,
					&command->open_client_session_lpae.
	...
	)
{
	...
	bool in_user_space = connection->owner != TF_CONNECTION_OWNER_KERNEL;
	struct tf_command_register_shared_memory_schannel *msg =
	...
	msg->message_size
		= (offsetof(struct tf_command_register_shared_memory_schannel,
						shared_mem_descriptors) -
			sizeof(struct tf_command_header)) / 4;
	...
	if (msg->shared_mem_size == 0) {
		/* Empty shared mem */
		msg->shared_mem_start_offset = shmem_addr;
	} else {
		u32 descriptor_count;
...
#ifdef SUPPORT_LARGE_PHYSICAL_ADDRESS_EXTENSION
	if (soc_is_omap54xx() || soc_is_dra7xx()) { /* u62 descriptor */
		 msg->message_size += descriptor_count * 2;
	} else
#endif /* SUPPORT_LARGE_PHYSICAL_ADDRESS_EXTENSION */
	{ /* u32 descriptor */
...
#ifdef SUPPORT_LARGE_PHYSICAL_ADDRESS_EXTENSION
	if (soc_is_omap54xx() || soc_is_dra7xx()) {
		command->invoke_client_command.message_size =
			(sizeof(struct tf_command_invoke_client_command_lpae) -
				sizeof(struct tf_command_header)) / 4;
	} else
#endif
	{
...
#ifdef SUPPORT_LARGE_PHYSICAL_ADDRESS_EXTENSION
			if (soc_is_omap54xx() || soc_is_dra7xx()) {
				error = tf_map_temp_shmem_lpae(
					connection,
					&command->invoke_client_command_lpae.
					params[i].temp_memref,
