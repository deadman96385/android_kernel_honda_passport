 ...
 */
static int tf_device_register(struct platform_device *pdev)
{
	int error = 0;
	struct tf_device *dev = &g_tf_dev;

	dprintk(KERN_INFO "%s()\n", __func__);
	if (dev->workspace_size < 8*SZ_1M) {
		printk(KERN_ERR
			"%s(): workspace size should be atleast 8MBytes\n", __func__);
		error = -EINVAL;
...
#ifdef CONFIG_TF_CRYPTO_RNG
	/*
	 * Setup /dev/hwrng using C_GenerateRandom() API
	 */
	error = tf_crypto_rng_init();
	if (error)
		goto rng_failed;
#endif
...
static void tf_adjust_command_format(union tf_command *pcommand,
	...
	u32 temp_memref_deptr_size =
		sizeof(((struct tf_command_param_temp_memref_lpae *) (0))->
		descriptor);
	u32 schannel_temp_memref_deptr_size =
		sizeof(((struct tf_tee_command_param_temp_memref *) (0))->
		descriptor);
	int i;
	...
	case TF_TEE_MESSAGE_TYPE_OPEN_CLIENT_SESSION:
		memcpy((void *)pcommand, (void *)pscommand,
	        ...
	        memcpy(&pcommand->open_client_session_lpae.login_type,
			&pscommand->open_client_session.login_type,
			sizeof(struct tf_command_open_client_session)-
			offsetof(struct tf_command_open_client_session,
				login_type));
		for (i = 0; i < 4; i++) {
                        int param_type;
			param_type = TF_GET_PARAM_TYPE(
				pscommand->open_client_session.
					param_types, i);
			if ((param_type & (TF_PARAM_TYPE_MEMREF_FLAG |
				...
				pcommand->open_client_session_lpae.params[i].
					temp_memref.descriptor =
				pscommand->open_client_session.params[i].
					temp_memref.descriptor;
				pcommand->open_client_session_lpae.params[i].
					temp_memref.size =
				pscommand->open_client_session.params[i].
	...
	unsigned long ioctl_param)
{
	...
	case IOCTL_TF_ION_REGISTER: {
		...
		/*
		 * TODO: We should use a reference count
		 * on this handle in order
		 * to not unregistered it while using it.
		 */
		return (long)ion_import_fd(
			connection->ion_client,
			ion_register);
	}
...
*/
		if (copy_from_user(&scommand,
				(union tf_tee_command *)ioctl_param,
				command_size * sizeof(u32))) {
			result = -EFAULT;
			goto exit;
		}
...
/*----------------------------------------------------------------------------*/
static int tf_device_probe(struct platform_device *pdev)
{
	dprintk(KERN_INFO "%s\n", __func__);
	return tf_device_register(pdev);
}

static int tf_device_remove(struct platform_device *pdev)
{
	omap_secure_dispatcher_switch(NULL);
	return 0;
}

static struct platform_device tf_device_0 = {
	.name = "tf_device",
	.id = 0,
	.dev = {
	 },
...
static int __init tf_reserved_mem_setup(struct reserved_mem *rmem)
{
	dprintk(KERN_INFO
		"Reserved memory: reserved memory for tf_driver at %x, size %ld MiB\n",
			rmem->base, (unsigned long)rmem->size / SZ_1M);

	g_tf_dev.workspace_size = (u32)rmem->size;
	g_tf_dev.workspace_addr = (u32)rmem->base;
	return 0;
}
RESERVEDMEM_OF_DECLARE(tf, "sec-rsvd", tf_reserved_mem_setup);
...
static int __init tf_device_init(void)
{
	int ret;
	dprintk(KERN_INFO "%s\n", __func__);

	ret = platform_driver_register(&tf_device_driver);
	if (ret != 0) {
		printk(KERN_ERR "platform_driver_register failed (%d)\n", ret);
	} else {
		ret = platform_device_register(&tf_device_0);
		if (ret != 0) {
			printk(KERN_ERR "platform_device_register failed (%d)\n", ret);
			platform_driver_unregister(&tf_device_driver);
		}
	}
	return ret;
}

static void __exit tf_device_exit(void)
{
	platform_device_unregister(&tf_device_0);
	platform_driver_unregister(&tf_device_driver);
}

module_init(tf_device_init);
module_exit(tf_device_exit);
