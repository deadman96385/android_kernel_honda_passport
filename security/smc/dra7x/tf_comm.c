...
#define L1_COARSE_DESCRIPTOR_BASE_LPAE			(0x0000000000000003ULL)
#define L1_COARSE_DESCRIPTOR_ADDR_MASK_LPAE	(0x000000FFFFFFF000ULL)
...
#define L3_PAGE_DESCRIPTOR_AP_APX_READ_LPAE			(0xC0)
#define L3_PAGE_DESCRIPTOR_AP_APX_READ_WRITE_LPAE	(0x40)
...
/*
 * AF = 1
 * ShareAbility=0b10 (outter shareable)
 * Ns=1
 */
#define L3_PAGE_DESCRIPTOR_BASE_LPAE		(0x0000000000000623ULL)
/*
...
#define L3_LPAE_ATTR_INDEX_MASK		(0b111<<2)
#define L3_LPAE_STRONGLY_ORDERED		(0b000<<2)
#define L3_LPAE_NON_CACHEABLE			(0b100<<2)
#define L3_LPAE_INNER_WRITE_THROUGH	(0b101<<2)
#define L3_LPAE_INNER_WRITE_BACK		(0b011<<2)
#define L3_LPAE_INNER_WRITE_ALLOC	(0b111<<2)
#define L3_LPAE_DEVICE					(0b001<<2)

#define SO_OR_DEV_MEM(x) \
...
#define SO_OR_DEV_MEM_LPAE(x) \
	((((x) & L3_LPAE_ATTR_INDEX_MASK) == L3_LPAE_STRONGLY_ORDERED) || \
	 (((x) & L3_LPAE_ATTR_INDEX_MASK) == L3_LPAE_DEVICE))
...
/* For short-descriptors */
#define SECT_ATTR_INDEX_MASK \
	(PMD_SECT_TEX(1) | PMD_SECT_CACHEABLE | PMD_SECT_BUFFERABLE)
#define PAGE_ATTR_INDEX_MASK       (0b111<<2)
	...
	u32 type)
{
	...
	int coarse_page_table_per_page;
	int coarse_page_table_size;
	...
	if (!(list_empty(&(alloc_context->free_coarse_page_tables)))) {
		...
		if (array == NULL) {
			dprintk(KERN_ERR
				"%s(%p): failed to allocate a table array\n",
				__func__, alloc_context);
			return NULL;
		}
		...
		if (soc_is_omap54xx() || soc_is_dra7xx()) {
			coarse_page_table_per_page = 1;
			coarse_page_table_size = SIZE_4KB;
		} else
#endif
		{
			coarse_page_table_per_page = 4;
			coarse_page_table_size = SIZE_1KB;
		}

		/* initialize the coarse page table descriptors */
		for (i = 0; i < coarse_page_table_per_page; i++) {
			INIT_LIST_HEAD(&(array->coarse_page_tables[i].list));
			array->coarse_page_tables[i].descriptors =
				page + (i * coarse_page_table_size);
	...
	u32 coarse_pg_table_descriptors[256])
{
	...
	if (CACHE_S(info) && (CACHE_DSIZE(info) & (1 << 11))) {
		dprintk(KERN_DEBUG "%s(): V31-12 added to descriptor\n",
			__func__);
...
#ifdef SUPPORT_LARGE_PHYSICAL_ADDRESS_EXTENSION
/*
 * Returns the L2 coarse page descriptor for
 * a coarse page table located at address coarse_pg_table_descriptors
 */
u64 tf_get_l1_coarse_descriptor_lpae(
	u64 coarse_pg_table_descriptors[128])
{
	u64 descriptor = L1_COARSE_DESCRIPTOR_BASE_LPAE |
		(virt_to_phys((void *) coarse_pg_table_descriptors) &
			L1_COARSE_DESCRIPTOR_ADDR_MASK_LPAE);

	return descriptor;
}
#endif

#define dprintk_desc(args...) dprintk(args)
...
#ifdef SUPPORT_LARGE_PHYSICAL_ADDRESS_EXTENSION
u64 tf_get_l3_descriptor_common_lpae(u32 vaddr, struct mm_struct *mm)
{
	pgd_t *pgd;
	pud_t *pud;
	pmd_t *pmd;
	...
	dprintk_desc(KERN_INFO "VirtAddr = %x\n", vaddr);
	pgd = pgd_offset(mm, vaddr);
	dprintk_desc(KERN_INFO "pgd = %x, value=%016llx\n", (unsigned int) pgd,
		(u64) pgd_val(*pgd));
	if (pgd_none(*pgd))
		goto error;
	pud = pud_offset(pgd, vaddr);
	dprintk_desc(KERN_INFO "pud = %x, value=%016llx\n", (unsigned int) pud,
		(u64) pud_val(*pud));
	if (pud_none(*pud))
		goto error;
	pmd = pmd_offset(pud, vaddr);
	dprintk_desc(KERN_INFO "pmd = %x, value=%016llx\n", (unsigned int) pmd,
		(u64) pmd_val(*pmd));
	if (pmd_none(*pmd))
		goto error;

	if (PMD_TYPE_SECT == (PMD_TYPE_MASK & pmd_val(*pmd))) {
#ifdef CONFIG_ARM_LPAE
		u64 pmd_attr = (*pmd) & L3_LPAE_ATTR_INDEX_MASK;
#else
		u32 pmd_attr = (*pmd) & SECT_ATTR_INDEX_MASK;
#endif
		...
		case PMD_SECT_UNCACHED:
			descriptor |= L3_LPAE_STRONGLY_ORDERED;
			break;
		case PMD_SECT_BUFFERED:
			descriptor |= L3_LPAE_NON_CACHEABLE;
			break;
		case PMD_SECT_WT:
			descriptor |= L3_LPAE_INNER_WRITE_THROUGH;
			break;
		case PMD_SECT_WB:
			descriptor |= L3_LPAE_INNER_WRITE_BACK;
			break;
		case PMD_SECT_WBWA:
			descriptor |= L3_LPAE_INNER_WRITE_ALLOC;
			break;
		default:
			dprintk(KERN_ERR "%s(): Bad section attributes\n",
				__func__);
			goto error;
		}
	} else if (PMD_TYPE_TABLE == (PMD_TYPE_MASK & pmd_val(*pmd))) {
		/* We have a table */
		ptep = pte_offset_map(pmd, vaddr);
		if (pte_present(*ptep)) {
#ifdef CONFIG_ARM_LPAE
			u64 ptep_attr = (*ptep) & L3_LPAE_ATTR_INDEX_MASK;
#else
			u32 ptep_attr = (*ptep) & PAGE_ATTR_INDEX_MASK;
#endif

			dprintk_desc(KERN_INFO "L3 descr=%x\n",
				(unsigned int) *ptep);
			...
			case L_PTE_MT_DEV_SHARED:
				descriptor |= L3_LPAE_DEVICE;
				break;
			default:
				dprintk(KERN_ERR "%s(): Bad table attributes\n",
					__func__);
				pte_unmap(ptep);
				goto error;
			}
...
#ifdef SUPPORT_LARGE_PHYSICAL_ADDRESS_EXTENSION
/*
 * Changes an L3 page descriptor back to a pointer to a physical page
 */
inline struct page *tf_l3_page_descriptor_to_page_lpae(u64 l3_page_descriptor)
{
	return pte_page(l3_page_descriptor & L3_DESCRIPTOR_ADDR_MASK_LPAE);
}
#endif
...
static void tf_get_l2_page_descriptor(
	...
	u32 flags, struct mm_struct *mm, struct page *page)
{
...
#ifdef SUPPORT_LARGE_PHYSICAL_ADDRESS_EXTENSION
/*
 * Returns the L3 descriptor for the 4KB-aligned coarse page table. The address
 * must be in the kernel address space.
 */
static void tf_get_l3_page_descriptor(u64 *l3_page_descriptor,
	u32 flags, struct page *page)
{
	unsigned long page_vaddr;
	u64 descriptor;
	bool unmap_page = false;

	if (page == L3_DESCRIPTOR_FAULT)
		return;
	...
	if (page_vaddr == 0) {
		...
		page_vaddr = (unsigned long) kmap(page);
		if (page_vaddr == 0) {
			*l3_page_descriptor = L3_DESCRIPTOR_FAULT;
			dprintk(KERN_ERR "kmap returned 0\n");
			return;
		}
		unmap_page = true;
	...
	descriptor = tf_get_l3_descriptor_common_lpae(page_vaddr, &init_mm);
	if (descriptor == 0) {
		*l3_page_descriptor = L3_DESCRIPTOR_FAULT;
		if (unmap_page)
			kunmap(page);
	...
	descriptor |= L3_PAGE_DESCRIPTOR_BASE_LPAE;

	descriptor |= (page_to_phys(page) & L3_DESCRIPTOR_ADDR_MASK_LPAE);

	if (!(flags & TF_SHMEM_TYPE_WRITE))
		/* only read access */
		descriptor |= L3_PAGE_DESCRIPTOR_AP_APX_READ_LPAE;
	else
		/* read and write access */
		descriptor |= L3_PAGE_DESCRIPTOR_AP_APX_READ_WRITE_LPAE;

	if (unmap_page)
		kunmap(page);

	*l3_page_descriptor = descriptor;
}
#endif
	...
	u32 full_cleanup)
{
	...
	printk(KERN_DEBUG "%s(): number of coarse page tables=%d\n",
		__func__, shmem_desc->coarse_pg_table_count);

	for (coarse_page_index = 0;
		  coarse_page_index < shmem_desc->coarse_pg_table_count;
		  coarse_page_index++) {
		u32 j;
		struct tf_coarse_page_table *coarse_page =
			shmem_desc->coarse_pg_table[coarse_page_index];

		if (coarse_page != NULL) {
			u32 capacity = TF_DESCRIPTOR_TABLE_CAPACITY;
#ifdef SUPPORT_LARGE_PHYSICAL_ADDRESS_EXTENSION
			if (soc_is_omap54xx() || soc_is_dra7xx())
				capacity = TF_DESCRIPTOR_TABLE_CAPACITY_LPAE;
#endif

			for (j = 0; j < capacity; j += 8) {
				int k;
				printk(KERN_DEBUG "	  ");
				...
				if (soc_is_omap54xx() || soc_is_dra7xx()) {
					for (k = j; k < j + 8; k++)
						printk(KERN_DEBUG "0x%016llX ",
					((u64 *)(coarse_page->descriptors))[k]);
				} else
#endif
				{
					for (k = j; k < j + 8; k++)
						printk(KERN_DEBUG "0x%08X ",
					((u32 *)(coarse_page->descriptors))[k]);
				}
	...
	for (coarse_page_index = 0;
		...
		if (soc_is_omap54xx() || soc_is_dra7xx()) {
			for (j = 0;
				  j < TF_DESCRIPTOR_TABLE_CAPACITY_LPAE;
				  j++) {
				u64 l3_page_descriptor =
					((u64 *)(coarse_page->descriptors))[j];

				if (l3_page_descriptor != L3_DESCRIPTOR_FAULT) {
					struct page *page =
					tf_l3_page_descriptor_to_page_lpae(
						l3_page_descriptor);
	...
	struct page **pages_table)
{
	...
	int ret = 0;
	unsigned int info = read_cpuid(CPUID_CACHETYPE);
	int max_coarse_pages;
	int max_coarse_page_mapped_size;
	int descriptor_table_capacity;
	int descriptor_table_capacity_bit_shift;
	int descriptor_table_capacity_mask;
	int descriptor_table_size;

#ifdef SUPPORT_LARGE_PHYSICAL_ADDRESS_EXTENSION
	if (soc_is_omap54xx() || soc_is_dra7xx()) {
		max_coarse_pages = TF_MAX_COARSE_PAGES_LPAE;
		max_coarse_page_mapped_size =
			TF_MAX_COARSE_PAGE_MAPPED_SIZE_LPAE;
		descriptor_table_capacity = TF_DESCRIPTOR_TABLE_CAPACITY_LPAE;
		descriptor_table_capacity_bit_shift =
			TF_DESCRIPTOR_TABLE_CAPACITY_BIT_SHIFT_LPAE;
		descriptor_table_capacity_mask =
			TF_DESCRIPTOR_TABLE_CAPACITY_MASK_LPAE;
		descriptor_table_size =
			TF_DESCRIPTOR_TABLE_CAPACITY_LPAE * sizeof(u64);
	} else
#endif
	{
		max_coarse_pages = TF_MAX_COARSE_PAGES;
		max_coarse_page_mapped_size = TF_MAX_COARSE_PAGE_MAPPED_SIZE;
		descriptor_table_capacity = TF_DESCRIPTOR_TABLE_CAPACITY;
		descriptor_table_capacity_bit_shift =
	...
	dprintk(KERN_INFO
		"%s(%p, bufr=0x%08X, size=0x%08X, user=%01x, flags = 0x%08x)\n",
		__func__,
		shmem_desc,
		buffer,
		buffer_size,
	 ...
	 */
	if (page_count > (descriptor_table_capacity * max_coarse_pages)) {
		dprintk(KERN_ERR
			"%s(%p): %u pages required to map shared memory!\n",
			__func__,
	...
	/* coarse page describe 256 pages or 512 pages for LPAE*/
	coarse_page_count = ((page_count + descriptor_table_capacity_mask) >>
			descriptor_table_capacity_bit_shift);
	dprintk(KERN_INFO
					 "%s(%p, coarse_page_count=%d)\n",
					 __func__,
					 shmem_desc,
					 coarse_page_count);
	...
	*buffer_start_offset = (buffer & ~PAGE_MASK) |
	...
	for (coarse_page_index = 0;
		...
		u32 buffer_offset_vaddr = buffer +
			(coarse_page_index * max_coarse_page_mapped_size);
		u32 pages_to_get;
		 ...
		 */
		pages_to_get =
			(page_count >> descriptor_table_capacity_bit_shift) ?
				descriptor_table_capacity : page_count;
		 ...
		 * Set the fault descriptors (each descriptor is 4/8 bytes long)
		 */
		memset(coarse_pg_table->descriptors, 0, descriptor_table_size);
		...
		if (in_user_space) {
			...
			for (j = page_shift; j < page_shift + pages; j++) {
#ifdef SUPPORT_LARGE_PHYSICAL_ADDRESS_EXTENSION
				if (soc_is_omap54xx() || soc_is_dra7xx()) {
					u64 *desc = &((u64 *)(coarse_pg_table->
						descriptors))[j];
					/* Get the actual L3 descriptors */
					tf_get_l3_page_descriptor(desc,
						flags, pages_table[j]);

					/*
					 * Reject Strongly-Ordered or Device
					 */
					if (SO_OR_DEV_MEM_LPAE(*desc)) {
						ret = -EFAULT;
						goto error;
					}
				} else
#endif
				{
					...
					u32 *desc = &((u32 *)(coarse_pg_table->
						descriptors))[j];

					tf_get_l2_page_descriptor(desc,
						flags, current->mm,
						pages_table[j]);
...
#ifdef SUPPORT_LARGE_PHYSICAL_ADDRESS_EXTENSION
		if (soc_is_omap54xx() || soc_is_dra7xx())
			((u64 *)descriptors)[coarse_page_index] =
				tf_get_l1_coarse_descriptor_lpae(
					coarse_pg_table->descriptors);
		else
#endif
			((u32 *)descriptors)[coarse_page_index] =
